...
Далее можно выполнять запросы к API (успел сделать только первый эндпойнт - `POST /links`). Если не меняли файл .env -
адрес API будет таким:
`http://localhost:58080/api` (в локальном окружении). Т.е. адрес первого эндпойнта
будет `http://localhost:58080/api/links`
